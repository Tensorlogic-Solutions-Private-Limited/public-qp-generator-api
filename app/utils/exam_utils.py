...
async def check_existing_design(db: AsyncSession, design_name: str):
    existing_design = await db.scalar(select(Design).where(Design.dm_design_name == design_name))
    if existing_design:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Design with this name already exists.")
    ...
    def build_base_query(filter_field, code_value):
        return (
            ...
            .where(
                ...
                Taxonomy.subject.has(smt_subject_code=subject_code),
                Taxonomy.medium.has(mmt_medium_code=medium_code),
